"""Herramienta independiente: Actualizar Efectivo Productivo."""
...
def actualizar_efectivo_desde_access(
    ...
    """Sincroniza la colección EEPP desde una tabla de Access."""
    ...
    log_message("Iniciando sincronización de Efectivo Productivo desde Access...")
    ...
    try:
        ...
        log_message(
            ...
            f"omitidos: {skipped}, errores: {errors}, total procesado: {processed}"
        )
        update_progress(processed)
...
class ActualizarEfectivoWindow(BaseToolWindow):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(frame_origen, text="Ruta del archivo:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ttk.Entry(frame_origen, textvariable=self.path_var, width=70).grid(
            row=0, column=1, padx=5, pady=5, sticky="we"
        )

        ttk.Label(frame_origen, text="Tabla:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ttk.Entry(frame_origen, textvariable=self.table_var, width=30).grid(
            row=1, column=1, padx=5, pady=5, sticky="w"
        )

        self.actualizar_btn = ttk.Button(
            frame_origen,
            text="⬆️ Ejecutar actualización",
        ...
        self.log_text = tk.Text(log_frame, height=12, state="disabled", wrap="word", background="white")
    ...
    def _run_sync(self, mdb_path: str, table: str) -> None:
        try:
            actualizar_efectivo_desde_access(self.db, mdb_path, table, self.log_text, self.progress)
        finally:
            try:
...
def abrir_actualizar_efectivo(parent: tk.Widget, db: firestore.Client) -> None:
    """Abre la ventana de Actualizar Efectivo Productivo y evita duplicados."""
    root = parent.winfo_toplevel()
    for window in root.winfo_children():
        if isinstance(window, ActualizarEfectivoWindow):
            window.lift()
            window.focus_set()
            return

    ActualizarEfectivoWindow(parent, db)
